...
// UseCaseHandler provides a convenient interface for Pasien use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Pasien data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Pasien, error) {
	res := Pasien{}

	// check permission
	err := u.Ctx.ValidatePermission("pasien.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Pasien data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("pasien.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &Pasien{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &Pasien{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data Pasien with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("pasien.create")
	if err != nil {
		return err
	}
...
// UpdateByID updates the Pasien data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("pasien.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the Pasien data for the specified ID with specified parameters.
...
// DeleteByID deletes the Pasien data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("pasien.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update Pasien data.
func (u *UseCaseHandler) setDefaultValue(old Pasien) error {
	if !old.ID.Valid {
