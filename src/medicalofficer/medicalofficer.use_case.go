...
// UseCaseHandler provides a convenient interface for MedicalOfficer use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the MedicalOfficer data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (MedicalOfficer, error) {
	res := MedicalOfficer{}

	// check permission
	err := u.Ctx.ValidatePermission("medical_officers.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of MedicalOfficer data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("medical_officers.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &MedicalOfficer{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &MedicalOfficer{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data MedicalOfficer with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("medical_officers.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(MedicalOfficer{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the MedicalOfficer data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("medical_officers.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the MedicalOfficer data for the specified ID with specified parameters.
...
// DeleteByID deletes the MedicalOfficer data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("medical_officers.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update MedicalOfficer data.
func (u *UseCaseHandler) setDefaultValue(old MedicalOfficer) error {
	if !old.ID.Valid {
