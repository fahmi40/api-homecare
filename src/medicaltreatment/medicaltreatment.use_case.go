...
// UseCaseHandler provides a convenient interface for MedicalTreatment use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the MedicalTreatment data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (MedicalTreatment, error) {
	res := MedicalTreatment{}

	// check permission
	err := u.Ctx.ValidatePermission("medical_treatments.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of MedicalTreatment data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("medical_treatments.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &MedicalTreatment{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &MedicalTreatment{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data MedicalTreatment with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("medical_treatments.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(MedicalTreatment{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the MedicalTreatment data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("medical_treatments.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the MedicalTreatment data for the specified ID with specified parameters.
...
// DeleteByID deletes the MedicalTreatment data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("medical_treatments.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update MedicalTreatment data.
func (u *UseCaseHandler) setDefaultValue(old MedicalTreatment) error {
	if !old.ID.Valid {
