...
// UseCaseHandler provides a convenient interface for Transaction use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Transaction data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Transaction, error) {
	res := Transaction{}

	// check permission
	err := u.Ctx.ValidatePermission("transactions.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Transaction data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("transactions.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &Transaction{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &Transaction{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data Transaction with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("transactions.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(Transaction{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the Transaction data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("transactions.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the Transaction data for the specified ID with specified parameters.
...
// DeleteByID deletes the Transaction data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("transactions.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update Transaction data.
func (u *UseCaseHandler) setDefaultValue(old Transaction) error {
	if !old.ID.Valid {
