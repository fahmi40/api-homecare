...
// RESTAPIHandler provides a convenient interface for Schedule REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the Schedule REST API handler.
...
// GetByID is the REST API handler for `GET /api/schedules/{id}`.
...
// Get is the REST API handler for `GET /api/schedules`.
...
// Create is the REST API handler for `POST /api/schedules`.
...
// UpdateByID is the REST API handler for `PUT /api/schedules/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/schedules/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/schedules/{id}`.
