...
// UseCaseHandler provides a convenient interface for Schedule use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Schedule data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Schedule, error) {
	res := Schedule{}

	// check permission
	err := u.Ctx.ValidatePermission("schedules.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Schedule data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("schedules.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &Schedule{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &Schedule{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data Schedule with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("schedules.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(Schedule{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the Schedule data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("schedules.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the Schedule data for the specified ID with specified parameters.
...
// DeleteByID deletes the Schedule data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("schedules.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update Schedule data.
func (u *UseCaseHandler) setDefaultValue(old Schedule) error {
	if !old.ID.Valid {
