...
// UseCaseHandler provides a convenient interface for TransactionDetail use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the TransactionDetail data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (TransactionDetail, error) {
	res := TransactionDetail{}

	// check permission
	err := u.Ctx.ValidatePermission("transaction_details.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of TransactionDetail data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("transaction_details.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &TransactionDetail{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &TransactionDetail{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data TransactionDetail with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("transaction_details.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(TransactionDetail{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the TransactionDetail data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("transaction_details.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the TransactionDetail data for the specified ID with specified parameters.
...
// DeleteByID deletes the TransactionDetail data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("transaction_details.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update TransactionDetail data.
func (u *UseCaseHandler) setDefaultValue(old TransactionDetail) error {
	if !old.ID.Valid {
