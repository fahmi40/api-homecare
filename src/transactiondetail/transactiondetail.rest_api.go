...
// RESTAPIHandler provides a convenient interface for TransactionDetail REST API handler.
type RESTAPIHandler struct {
	UseCase UseCaseHandler
}

// injectDeps inject the dependencies of the TransactionDetail REST API handler.
...
// GetByID is the REST API handler for `GET /api/transaction_details/{id}`.
...
// Get is the REST API handler for `GET /api/transaction_details`.
...
// Create is the REST API handler for `POST /api/transaction_details`.
...
// UpdateByID is the REST API handler for `PUT /api/transaction_details/{id}`.
...
// PartiallyUpdateByID is the REST API handler for `PATCH /api/transaction_details/{id}`.
...
// DeleteByID is the REST API handler for `DELETE /api/transaction_details/{id}`.
func (r *RESTAPIHandler) DeleteByID(c *fiber.Ctx) error {
	...
	res := map[string]any{
		...
		"message": r.UseCase.Ctx.Trans("deleted", map[string]string{
			"transaction_details": p.EndPoint(),
			"id":                  c.Params("id"),
		}),
	}
	return c.JSON(res)
