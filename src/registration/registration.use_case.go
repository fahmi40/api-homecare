...
// UseCaseHandler provides a convenient interface for Registration use case, use UseCase to access UseCaseHandler.
...
// GetByID returns the Registration data for the specified ID.
func (u UseCaseHandler) GetByID(id string) (Registration, error) {
	res := Registration{}

	// check permission
	err := u.Ctx.ValidatePermission("registrations.detail")
	if err != nil {
		return res, err
	}
...
// Get returns the list of Registration data.
func (u UseCaseHandler) Get() (app.ListModel, error) {
	res := app.ListModel{}

	// check permission
	err := u.Ctx.ValidatePermission("registrations.list")
	if err != nil {
		return res, err
	}
	// get from cache and return if exists
	...
	res.Count,
		...
		err = app.Query().PaginationInfo(tx, &Registration{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
	...
	// find data
	data, err := app.Query().Find(tx, &Registration{}, u.Query)
	if err != nil {
		return res, app.Error().New(http.StatusInternalServerError, err.Error())
	}
...
// Create creates a new data Registration with specified parameters.
func (u UseCaseHandler) Create(p *ParamCreate) error {

	// check permission
	err := u.Ctx.ValidatePermission("registrations.create")
	if err != nil {
		return err
	}
	...
	err = p.setDefaultValue(Registration{})
	if err != nil {
		return err
	}
...
// UpdateByID updates the Registration data for the specified ID with specified parameters.
func (u UseCaseHandler) UpdateByID(id string, p *ParamUpdate) error {

	// check permission
	err := u.Ctx.ValidatePermission("registrations.edit")
	if err != nil {
		return err
	}
...
// PartiallyUpdateByID updates the Registration data for the specified ID with specified parameters.
...
// DeleteByID deletes the Registration data for the specified ID.
func (u UseCaseHandler) DeleteByID(id string, p *ParamDelete) error {

	// check permission
	err := u.Ctx.ValidatePermission("registrations.delete")
	if err != nil {
		return err
	}
...
// setDefaultValue set default value of undefined field when create or update Registration data.
func (u *UseCaseHandler) setDefaultValue(old Registration) error {
	if !old.ID.Valid {
